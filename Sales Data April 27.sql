...
(SELECT 
        ...
        DENSE_RANK() OVER(PARTITION BY product_category ORDER BY SUM(total_sales) DESC) DRNK
FROM sales_data
...
SELECT 
	    ...
	    MARKET_PLACE,
        SUM(total_sales) as sales,
        DENSE_RANK() OVER(PARTITION BY MARKET_PLACE ORDER BY SUM(total_sales) DESC) DRNK
FROM sales_data
GROUP BY product_category,seller_id,MARKET_PLACE)
SELECT product_category,seller_id,MARKET_PLACE FROM CTE WHERE DRNK <=3
